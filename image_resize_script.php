...
if (strtolower($_SERVER['HTTP_HOST']) == "192.168.200.1") {
    $src = get_request("src", "");
    $h = get_request("h", "");
    $w = get_request("w", "");
    echo '<img src="' . $src . '" height="' . $h . '" width="' . $w . '"/>';
    //$cache_dir = 'upload_unit_image/';
    //$mime_type = mime_type($src);
    //show_image($mime_type, $src, $cache_dir);
} else {
    $imageFilters = array(
